...
class NetworkA5:
    def __init__(self,sizes, batch_size, epoch_num, 
        ...
        if use_trained_params:
            ...
            self.W2 = np.sqrt(1/self.pool_output_size) * np.random.randn(self.pool_output_size, self.sizes[1]) #(pool,100)
            self.b2 = np.sqrt(1/self.pool_output_size) * np.random.randn(self.sizes[1])
            #　Fullyconnected層重み(出力層)
            self.W3 = np.sqrt(1/sizes[1]) * np.random.randn(self.sizes[1], self.sizes[2])
    ...
    def gradient(self, x, y_one_hot):
        ...
        grads['W2'] = self.layers['FullyConnected1'].dW
        grads['b2'] = self.layers['FullyConnected1'].db
        grads['W3'] = self.layers['FullyConnected2'].dW
        grads['b3'] = self.layers['FullyConnected2'].db
    ...
    def save_params(self, params):
        ...
        if self.use_bn:
            np.savez( os.path.join(path, filename ),\
                    W1=params['W1'], b1=params['b1'], W2=params['W2'], b2=params['b2'], W3=params['W3'], b3=params['b3'],gamma=params['gamma'], beta=params['beta'], running_mean=params['running_mean'], running_var=params['running_var'])
        else:
            np.savez( os.path.join(path, filename ),\
                    W1=params['W1'], b1=params['b1'], W2=params['W2'], b2=params['b2'],  W3=params['W3'], b3=params['b3'],gamma=params['gamma'], beta=params['beta'])
